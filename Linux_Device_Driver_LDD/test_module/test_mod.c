...
{
    kernel_buffer = kmalloc(BUF_SIZE, GFP_KERNEL);//dynamically allocating memory
    if (!kernel_buffer) {
        pr_err("testmod: failed to allocate memory\n");
        return -ENOMEM;
    }

    major = register_chrdev(0, DEVICE_NAME, &fops);//registering char chrdev
    if (major < 0) {
...
{
    unregister_chrdev(major, DEVICE_NAME);//unregisteing char device 
    kfree(kernel_buffer);
